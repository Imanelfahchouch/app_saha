...
export default function MapPage(props) {
  ...
  const [isGeocoding, setIsGeocoding] = useState(false); // État pour éviter le spam API

  const handleRouteSelect = (etab) => {
    if (props.showToast) props.showToast(`🗺️ Itinéraire vers ${etab.nom}`, "info");
  };

  // Filtrage LOCAL
  const dataToShow = useMemo(() => {
    let base = isNearMeActive ? nearbyEstablishments : (props.allEtablissements || []);
    ...
    if (props.activeFilters?.length > 0) {
      base = base.filter(e => props.activeFilters.includes(e.type));
    }
    if (props.activeStatusFilter && props.activeStatusFilter !== 'all') {
      base = base.filter(e => e.etat === props.activeStatusFilter);
    }
    return base;
  }, [isNearMeActive, nearbyEstablishments, props.allEtablissements, mapSearchQuery, props.activeFilters, props.activeStatusFilter]);
  ...
  const handleNearMe = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (props.showToast) props.showToast(`📍 Position détectée`, "info");
        if (accuracy > 5000) { latitude = 34.2133; longitude = -4.0100; } // Fallback Taza
        ...
        try {
          const res = await fetch(`http://localhost:8000/api/etablissements/nearby?lat=${latitude}&lng=${longitude}&radius=5000`);
          if (!res.ok) throw new Error('Erreur API');
          const data = await res.json();
          setNearbyEstablishments(data);
          if (props.showToast) props.showToast(`✅ ${data.length} établissement(s) proche(s)`, "success");
        } catch (err) {
  ...
  // ✅ Géocodage Ville CORRIGÉ (avec debounce et gestion erreur)
  const handleCitySearchOSM = async (cityName) => {
    if (!cityName.trim() || isGeocoding) return;
    
    setIsGeocoding(true);
    try {
      // Ajout d'un petit délai pour éviter de bloquer Nominatim
      await new Promise(r => setTimeout(r, 500));
      
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(cityName + ', Maroc')}&limit=1`,
        { headers: { 'Accept-Language': 'fr', 'User-Agent': 'SAHA-App/1.0' } }
      );
      ...
      if (data?.[0]?.lat && data?.[0]?.lon) {
        ...
        if (props.showToast) props.showToast(`📍 Centré sur ${cityName}`, "success");
      } else {
        if (props.showToast) props.showToast(`❌ Ville "${cityName}" non trouvée`, "error");
      }
    } catch (err) {
      console.error('❌ Erreur géocodage:', err);
      if (props.showToast) props.showToast("Erreur de localisation de la ville", "error");
    } finally {
      setIsGeocoding(false);
    }
  };

  // Déclenche le géocodage quand on tape une ville connue
  useEffect(() => {
    const villes = ['rabat', 'casablanca', 'fes', 'marrakech', 'tanger', 'agadir', 'meknes', 'oujda', 'taza'];
    const query = mapSearchQuery?.toLowerCase().trim();
    
    // On cherche si la requête contient exactement le nom d'une ville
    const foundCity = villes.find(v => query === v || query.startsWith(v + ' '));
    
    if (foundCity && !isGeocoding) {
      handleCitySearchOSM(foundCity);
    }
  }, [mapSearchQuery]);

  // ✅ Expose les fonctions de contrôle de la carte au parent (App.js)
  // Cela permet à ListPage de dire "Va à cette position"
  useEffect(() => {
    if (props.onMapControlReady) {
      props.onMapControlReady({
        goToLocation: (lat, lng, z = 13) => {
  ...
  return (
    <>
          ...
          <div className="search-container" style={{ marginTop: 0, marginBottom: "1.5rem" }}>
            <div className="row g-3 align-items-center">
              ...
              <div className="col-lg-5">
                <div className="position-relative">
                  ...
                  <input 
                    ...
                    placeholder="Rechercher par nom ou ville (ex: Fes)..." 
                    value={mapSearchQuery} 
                    onChange={e => setMapSearchQuery(e.target.value)} 
                  />
                  {isGeocoding && (
                    <span className="spinner-border spinner-border-sm position-absolute" style={{ right: 10, top: '50%', transform: 'translateY(-50%)' }}></span>
                  )}
                </div>
              </div>

              <div className="col-lg-3">
                <select className="search-input-custom" value={props.activeStatusFilter || 'all'} onChange={e => props.setActiveStatusFilter?.(e.target.value)}>
              ...
              <div className="col-lg-2">
                <button className="btn-near-me w-100" onClick={handleNearMe} disabled={nearMeLoading} style={{ background: isNearMeActive ? 'var(--primary)' : undefined }}>
                  {nearMeLoading ? <><span className="spinner-border spinner-border-sm me-2" />Localisation...</> : <><i className="bi bi-crosshair me-2" />Near Me</>}
                </button>
              </div>

              <div className="col-lg-2">
                <button className="btn-search w-100" onClick={() => setMapSearchQuery('')}>
      ...
      {/* ✅ Map avec données filtrées LOCALEMENT */}
      <Map
        {...props} 
        ...
        <div className="container">
          ...
          <div className="row g-3">
            {dataToShow.length === 0 && isNearMeActive ? (
              <div className="col-12 text-center py-4 text-muted">Aucun établissement trouvé dans ce rayon (5km).</div>
            ) : (
              dataToShow.map(e => (
